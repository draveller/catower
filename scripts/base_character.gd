class_name BaseCharacter
extends CharacterBody2D

# 基础属性
@export_group("基础属性")
@export var team: String = "team1" # 队伍名称, 怪物会攻击非同队的怪物
@export var max_health: float = 100.0 # 最大生命值
@export var move_speed: float = 50.0 # 移动速度
@export var static_facing: int = 1 # 贴图静态朝向, -1表示向左, 1表示向右
@export var initial_facing: int = 1 # 初始动作朝向, -1表示向左, 1表示向右

@onready var attack_area: Area2D = $AttackArea
@onready var hurt_area: Area2D = $HurtArea
@onready var animated_sprite: AnimatedSprite2D = $AnimatedSprite2D
@onready var invincible_timer: Timer = $InvincibleTimer
@onready var health_bar: TextureProgressBar = $HealthBar

# 状态变量
var current_health: float # 当前生命值
# 物理常量
const GRAVITY: float = 980.0 # 重力加速度
const FRICTION: float = 1000.0 # 摩擦力值
# 状态机
var mc: MonsterStateMachine

# 初始化函数
func _ready() -> void:
    current_health = max_health
    # 根据初始朝向设置节点翻转
    turn_facing()
    # 初始化血量
    health_bar.value = 100.0
    # 初始化状态机
    mc = MonsterStateMachine.new(self)


# 物理处理函数
func _physics_process(delta: float) -> void:
    # 应用重力
    if not is_on_floor():
        velocity.y += GRAVITY * delta
    move_and_slide()
    mc.update(delta)


func turn_facing() -> void:
    if static_facing != initial_facing:
        var sprite = get_node("Sprite2D") if has_node("Sprite2D") else get_node("AnimatedSprite2D")
        sprite.flip_h = true

func idle() -> void:
    animated_sprite.play("idle")

func move() -> void:
    animated_sprite.play("move")
    velocity.x = initial_facing * move_speed

func attack() -> void:
    animated_sprite.play("attack")

func hurt(damage: int) -> void:
    if invincible_timer.get_time_left() > 0:
        return
    animated_sprite.play("hurt")
    current_health -= damage
    health_bar.value = current_health/max_health * max_health

    velocity.x = - initial_facing * 160.0
    velocity.y = -100.0
    invincible_timer.start()

func dead() -> void:
    animated_sprite.play("dead")
    velocity = Vector2.ZERO
    await get_tree().create_timer(1.5).timeout
    queue_free()


# 检查是否是敌人
func is_enemy(node: Node) -> bool:
    if node is BaseCharacter:
        return node.team and node.team != self.team
    if node is Area2D and node.owner is BaseCharacter:
        return node.owner.team and node.owner.team != self.team
    return false
